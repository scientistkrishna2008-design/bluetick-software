  ...
  return (
    <div className="w-full py-8 px-4 overflow-x-auto">
      <div className="min-w-[800px] flex items-center justify-between relative">
        ...
        {/* Progress Line */}
        <div 
          className="absolute left-0 top-1/2 -translate-y-1/2 h-1 bg-growbroo-500 z-0 transition-all duration-500"
            ...
            <div key={stage} className="relative z-10 flex flex-col items-center gap-2">
              <div 
                className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors ${
                  isCompleted 
                    ? "bg-growbroo-500 text-white" 
                    : isActive 
                      ? "bg-surface border-2 border-growbroo-500 text-growbroo-500" 
              ...
              <span className={`text-xs whitespace-nowrap font-medium ${
                isActive ? "text-growbroo-500" : isCompleted ? "text-white" : "text-gray-500"
              }`}>
                {stage}
              </span>
            </div>
